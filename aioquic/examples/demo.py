...
STATIC_ROOT = os.environ.get("STATIC_ROOT", os.path.join(ROOT, "templates"))
STATIC_URL = "/"
...
async def homepage2(request):
    """
    Simple homepage.
    """
    print(str(request))
    return templates.TemplateResponse("myvideo.mpd", {"request": request})

async def videoseg(request):
    """
    Simple homepage.
    """
    print(str(request))
    return FileResponse('./templates/' + request.path_params['video'])
...
starlette = Starlette(
    routes=[
        Route("/", homepage),
        Route("/myvideo.mpd", homepage2),
        # Route("/{video}", videoseg),
        Route("/{size:int}", padding),
        Route("/echo", echo, methods=["POST"]),
        Mount("/httpbin", WsgiToAsgi(httpbin.app)),
        Route("/logs", logs),
        WebSocketRoute("/ws", ws),
        Mount(STATIC_URL, StaticFiles(directory=STATIC_ROOT)),
    ]
)
